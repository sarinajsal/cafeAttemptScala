...
object AttemptTwo extends App {
  ...
  case class MenuItems(name: String, price: Double, isHot: Boolean, isFood: Boolean, isPrem: Boolean) //TODO: case classes should be in another file, "val" is default so not needed
  //TODO: Wouldn't this be a "MenuItem"? What was your reason behind "MenuItems"?

  val cola = MenuItems(name = "cola", price = 0.50, isHot = false, isFood = false, isPrem = false) //TODO: Using named parameters would make this clear
  ...
  val lobster = Lobster
  val gary = Lobster.overrideName("gary")

  println(s"vb ${gary.name}")
  println(lobster.price)
  ...
  val initialAlfie = Customer("Alfie", true, 6) //TODO: lowercase for vals
...
//      newVersion
  ...
  val alfieAfterOneCheck: Customer = checkCustomerStarsAndIncrement(initialAlfie)
 ...
 */
  ...
  def isHappyHour (menuItems: List[MenuItems]): Double = { //TODO: isHappyHour returns a Double? Sounds like it should return a Boolean, possibly rename
    val totalDrinkPrice =(menuItems.filter(item => !item.isFood).map(drinks => drinks.price).sum) //TODO: Yes! Good use of these functions, but what can item.isFood == false be simplified to?
  ...
  def bill (menuItems: List[MenuItems], priceAfterLoyalty: Double): String = {
    //val isHot: List[Boolean] =  menuItems.map(x => x.isHot) //list of bools //TODO: Instead of commenting list of bools, go ahead and give this val a type!
    val isFood = menuItems.map(x => x.isFood)
    val isPrem = menuItems.map(x => x.isPrem)
    val names: List[String] = menuItems.map(_.name)

    if (menuItems.exists(_.isHot)){ //TODO: could you do menuItems.exists(...)???
    ...
    else if(isFood.contains(true) && isPrem.contains(false)){
  ...
  def finalBillMethod(menuItemList: List[MenuItems], customer: Customer): Unit = {
    ...
    val fin = bill(menuItemList, loyalty)
      if (customer.camePreviousMonth == true){ //TODO: again could customer.camePreviousMonth == true be simplified?
        val customerWithNewLoyalty: Customer = checkCustomerStarsAndIncrement(customer) //TODO: updatedLoyalty is a Customer? Possibly rename or change type?
        println(s"The bill for ${customerWithNewLoyalty.name} is $fin. You now have ${customerWithNewLoyalty.numStars} loyalty stars!")
      }else{
  ...
  finalBillMethod(List(lobster,cola, coffee, lobster, steakSandwhich, cheeseSandwhich), initialAlfie)
  /*
