...
object Attempt extends App{
  ...
  case class MenuItems(val name: String, temp: Attempt.temp, val price: Double, foodType: Attempt.foodType)
  ...
  def billWHotFood(menuItems: List[MenuItems]):String = {
    ...
    val totalWOService = y.sum
      if (x.contains(Food) && temp.contains(Hot)){
        var hotFoodFinalService = totalWOService * 0.2.max(20)
          if(hotFoodFinalService >= 20.00) {hotFoodFinalService}
